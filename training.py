        ...
        for img_layer in image_layers:
            ...
            if image.ndim == 2:
                ...
                if image.shape[2] < self.in_chans:
                    ...
                    # Prefer fixed slice [z_start : z_start + in_chans] if available, else center
                    if self.z_start + self.in_chans <= image.shape[2]:
                        start = self.z_start
                    else:
                        mid = image.shape[2] // 2
...
def train_i3d_model(viewer: napari.Viewer,
    ...
    # Debug image export removed after validation phase
